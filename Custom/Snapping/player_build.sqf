...
if (_hasrequireditem) then {
	...
	s_building_snapping = player addAction ["<t color=""#0000ff"">Toggle Snapping</t>", "Custom\Snapping\player_toggleSnapping.sqf",false, 3, true, false, "",""];
	...
	while {_isOk} do {
		...
		if(((SnappingOffset select 2) > 15) or ((SnappingOffset select 2) < -15)) exitWith {
			_isOk = false;
			_cancel = true;
