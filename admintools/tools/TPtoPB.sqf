...
if (pselect5 != "exit") then
{
	_name = pselect5;
	{
		...
		{
			_x setVariable ["AllowTP",true,true];
			sleep 0.2;		
		
			hint format ["Teleporting %1", _name];
			_x setpos [6824.55, 2441.53, 0];
			titleText ["Welcome to the IDC Penalty Box.", "PLAIN DOWN"];titleFadeOut 2;
			sleep 0.5;
			
			sleep 0.2;
			_x setVariable ["AllowTP",false,true];			
		};
