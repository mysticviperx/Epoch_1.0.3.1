	...
	if (selecteditem!= "exitscript") then
	{
		_name = selecteditem;
		{
			if(format[name _x] == _name) then 
			{
				...
				if ((getPlayerUID _x) in _admins) exitwith { titleText ["Cannot spectate an admin!","PLAIN DOWN"];titleFadeOut 4; };
				if ((getPlayerUID _x) in _mods) exitwith { titleText ["Cannot spectate an moderator!","PLAIN DOWN"];titleFadeOut 4; };
