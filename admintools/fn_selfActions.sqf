...
if (!isNull cursorTarget and !_inVehicle and !_isPZombie and (player distance cursorTarget < _allowedDistance) and _canDo) then {	//Has some kind of target
	...
	_isModular = (_cursorTarget isKindOf "ModularItems" || _cursorTarget isKindOf "DZE_Housebase" || _cursorTarget isKindOf "Fortifications");
	...
	_player_lockUnlock_crtl = false;
	 ...
	 if (_canDo && (speed player <= 1) && (_cursorTarget isKindOf "Plastic_Pole_EP1_DZ")) then {
		 if (s_player_maintain_area < 0) then {
		 	...
		 	s_player_maintain_area_preview = player addAction ["<t color=""#ccffffff"">Maintain Area Preview</t>", "fixes\maintain_area.sqf", "preview", 5, false];
		 };
	 } else {
	...
	if(_isAlive) then {
		...
		// Building stuff?
		if (_isModular or _isDestructable or _isWreck or _isRemovable or _isWreckBuilding) then {
			
			_findNearestPoles = nearestObjects [player, ["Plastic_Pole_EP1_DZ"], 100];  // 100m range
			...
			if (_IsNearPlot == 0) then {  // No plot pole within 100m range. All bets are off.
				...
				// Find owner 
				_poleOwnerID = _nearestPole getVariable["CharacterID","0"];
			
				if(_hasToolbox and ("ItemCrowbar" in _itemsPlayer)) then {
			
					if (dayz_characterID == _poleOwnerID) then {  
							_player_deleteBuild = true;
						...
						if(_poleOwnerID in _friendlies) then {
							_player_deleteBuild = true;
						};  
					};
				};
				...
				if (_player_deleteBuild) then {
	           		if(_cursorTarget in DZE_DoorsLocked) exitwith  { _player_deleteBuild=false; cutText ["You must remove the lock to delete this item!", "PLAIN DOWN"]; };
	...
	if ((_typeOfCursorTarget in DZE_DoorsLocked) and _ownerID != "0"  and (player distance _cursorTarget < 3)) then {
			if ((getPlayerUID player) in _admins) then {
				if (s_player_showdoorcode < 0) then {
					s_player_showdoorcode = player addaction [format["<t color='#6699ff'>Door Code: %1</t>", _ownerID], "",_cursorTarget, 0, false, true, "", ""];	
				};
			};	
	} else {
		player removeAction s_player_showdoorcode;
		s_player_showdoorcode = -1;
	};
